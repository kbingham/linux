...
struct vsp1_dl_list *dbg_vsp1_dl_list_get(struct vsp1_dl_manager *dlm, const char *f, int l)
{
	...
	if (!list_empty(&dlm->free)) {
		...
		/* The display list chain must be initialised to ensure
		 * every DL can assert list_empty() if it is not in a chain
		 */
...
static void dbg__vsp1_dl_list_put(struct vsp1_dl_list *dl, const char *f, int l)
{
	...
	/* Release any linked display-lists which were chained
	 * for a single hw operation
	 */
	if (dl->has_chain)
 ...
 * the CPU. A frame end interrupt will only complete after the last display
...
int vsp1_dl_list_add_chain(struct vsp1_dl_list *head,
			   struct vsp1_dl_list *dl)
{
	/* chain lists are only available in header mode. */
...
static void vsp1_dl_list_complete_header(struct vsp1_dl_list *dl, bool is_last)
{
	...
	/* If this DL's chain is not empty, we are on a list, where
	 * the next item in the list is the DL entity which should
	 * be automatically queued by the hardware
	 */
...
void vsp1_dl_list_commit(struct vsp1_dl_list *dl)
{
	...
	if (dl->dlm->mode == VSP1_DL_MODE_HEADER) {
		...
		/* Complete the display list header for the head dl */
		vsp1_dl_list_complete_header(dl, list_empty(&dl->chain));

		list_for_each_entry(dl_child, &dl->chain, chain) {
			bool last = list_is_last(&dl_child->chain, &dl->chain);

			vsp1_dl_list_complete_header(dl_child, last);
		}

		/* Commit the head DL to hw. Chained headers will auto-start */
