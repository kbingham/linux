...
#define FDP1_NUMBER_JOBS 4
#define FDP1_NUMBER_BUFFERS (FDP1_NUMBER_JOBS*2)
...
static char *v4l2_field(enum v4l2_field f)
{
...
#define V4L2_CID_DEINT_MODE		(V4L2_CID_USER_BASE + 0x1002)
...
struct fdp1_dev {
	...
	/* User controls persist in the device */
				      ...
				      struct fdp1_job *job)
{
	...
	case FDP1_ADAPT2D3D:
		...
		WARN(1, "ADAPT_2D3D NOT YET SUPPORTED WITH NO STILL MASK!\n");
		break;
	case FDP1_FIXED3D:
...
void print_frame(struct fdp1_dev *fdp1,
		 struct fdp1_q_data *q,
		 struct vb2_v4l2_buffer *buf)
{
	struct vb2_buffer *vb2_buf = &buf->vb2_buf;
	struct v4l2_pix_format_mplane *f = &q->format;

	dprintk(fdp1, "Queue: %s: %dx%d numplanes %d\n",
		v4l2_field(f->field), f->width, f->height,
		 f->num_planes);

	dprintk(fdp1, "Buffer[%d] is %s\n",
		vb2_buf->index, v4l2_field(buf->field));
...

{
	...
	if (!job) {
		/*
		 * VINT calls us to see if we can queue another job.
...
static struct fdp1_job *prepare_job(struct fdp1_ctx *ctx)
{
	struct fdp1_dev *fdp1 = ctx->fdp1;
	struct fdp1_job *job;
	unsigned int buffers_required = ctx->deinterlace ? 2 : 1;
 ...
 * device_vinterrupt:
 *
 * Handles queueing the next job when the hardware is ready.
 */
static void device_vinterrupt(struct fdp1_dev *fdp1)
{
	struct fdp1_ctx *ctx;
	...
	if (ctx == NULL) {
		v4l2_err(&fdp1->v4l2_dev,
			"VINT after transaction finished\n");
		return;
	}
			     ...
			     enum vb2_buffer_state state)
{
	...
	/*
	 * free_buffer will call buf_done only when the last vb2_buffer
...
static const struct v4l2_ctrl_config fdp1_ctrl_deint_mode = {
	...
	.def = FDP1_PROGRESSIVE,
	.min = FDP1_PROGRESSIVE,
	.max = FDP1_NEXTFIELD,
};

static const struct v4l2_ctrl_config fdp1_ctrl_min_buffers = {
	.ops = &fdp1_ctrl_ops,
...
static int fdp1_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(q->type)) {
		if (q_data->format.field == V4L2_FIELD_NONE)
			...
			ctx->deint_mode = FDP1_FIXED2D;

		/* Debug - If ctrl set on dev - use that de-int mode */
		if (ctx->fdp1->deint_mode) {
			dprintk(ctx->fdp1, "Setting deint based on dev\n");
			ctx->deint_mode = ctx->fdp1->deint_mode;

			/* If we've forced PRG, Force FIELD_NONE */
			if (ctx->deint_mode == FDP1_PROGRESSIVE)
				q_data->format.field = V4L2_FIELD_NONE;
		}
	}
...
static int fdp1_open(struct file *file)
{
	...
	v4l2_ctrl_new_custom(&ctx->hdl, &fdp1_ctrl_min_buffers, NULL);
...
static irqreturn_t fdp1_irq_handler(int irq, void *dev_id)
{
	...
	if (FD1_CTL_IRQ_VINTE & int_status)
		device_vinterrupt(fdp1);
